...
func main() {
	...
	flag.BoolVar(&verbose, "verbose", true, "Print verbose config information")
	...
	mode := "controller"
	//fmt.Printf("masterurl: %s", masterURL)
	...
	// faasconfig 部署函数的配置，包含了自设置Qps burst阈值以及从yaml文件中读取的
	// kubeconfig 是k8s的配置 主要是从yaml文件中获取
	// config openfaas平台的配置
